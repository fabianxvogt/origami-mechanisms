...
The geometry, model, SVG, motion-loop, app-wiring, and production DOM-wiring tests cover contract bounds, separate rotation/position closure gates, rigid edge/hinge residuals, sampled collision classification, versioned round trips, SVG layers/metadata, timer cleanup, preview-only motion ownership, preview-cancel repaint, recovery-safe imports, and actual indexed SVG IDs through render and Save completion. An isolated browser run against source `1b3b6895b1029e43b9ffa748a31f6e58560d55cd` found a quoted indexed-ID lookup defect that blocked valid variant Save; the local repair is committed for independent browser recheck. These checks do not prove continuous collision freedom, thick-paper behavior, physical hinge clearance, successful paper folds, browser behavior for the repaired source, or full-v1 acceptance.

## Release status

The ordinary source is independently accepted for bounded publication at the reviewed source state `9a17a4608392494faa7a8573c4923f37f9f6f7d8`. A private Foldline Site version 1 is saved separately but remains undeployed; the later private version 2 remains unchanged until the repaired source passes isolated browser recheck. Browser verification of the repair, physical-fold checks, and any full-v1 claim remain open; sampled zero-thickness collision status is the only collision evidence provided.

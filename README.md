...
The geometry, model, SVG, motion-loop, app-wiring, and production DOM-wiring tests cover contract bounds, separate rotation/position closure gates, rigid edge/hinge residuals, sampled collision classification, versioned round trips, SVG layers/metadata, timer cleanup, preview-only motion ownership, preview-cancel repaint, recovery-safe imports, and actual indexed SVG IDs through render and Save completion. The c241 browser evidence also confirms four nonzero 3D creases at q=77°/+1 and q=101°/−1, readable labels, populated-library empty-state hiding, and bounded 390px controls. The public v4 smoke additionally confirmed a visible q=77°/+1 Save, settled reload retention, explicit Reopen, fresh JSON and SVG exports, and the 390px bounds. These checks do not prove continuous collision freedom, thick-paper behavior, physical hinge clearance, successful paper folds, device behavior, human readability, performance, or full-v1 acceptance.

## Release status

Classification: INCREMENTAL public preview. The bounded public product source is `c241d1d8937e552e5ae07f66ad63757f9fac33d5`; its reviewed ordinary source/docs are published on `fabianxvogt/origami-mechanisms` main. Foldline Site version 4 is publicly deployed at https://foldline-origami-mechanisms.fabian523417.chatgpt.site. The current product and Site version remain c241 and v4. Core Save/button/Enter/reload/Reopen/project/SVG/import/error-retention evidence, the c241 display browser gate, and the public Save/reload/Reopen/export smoke passed. This remains a bounded preview: sampled zero-thickness collision status is the only collision evidence, and no physical-fold, continuous-collision, device, human, performance, or full-v1 claim is made.

...
function saveVariant() { if (!state.solution?.valid) { announce("Fix the invalid state before saving a variant.", "error"); return; } const previewWasActive = state.previewQDeg !== null; stopMotion(); if (previewWasActive) renderAll(); const library = readLocalVariants(); const name = window.prompt("Name this variant", state.project.name || `Variant ${library.variants.length + 1}`); if (name === null) return; const saved = { ...cloneProject(state.project), name: name.trim().slice(0, 80) || `Variant ${library.variants.length + 1}`, savedAt: new Date().toISOString() }; const existingIndex = library.variants.findIndex((item) => projectKey(item) === projectKey(saved)); if (existingIndex >= 0) library.variants.splice(existingIndex, 1); library.variants.unshift(saved); if (writeLocalVariants(library.variants)) { state.project = saved; state.dirty = false; renderAll(); announce("Variant saved on this device.", "success"); } }
...
function bindEvents() {
  ...
  $("modal-close").addEventListener("click", closeModal); $("modal-cancel").addEventListener("click", closeModal); $("modal-confirm").addEventListener("click", confirmReplace); document.addEventListener("keydown", (event) => { if (event.key === "Escape" && !$("confirm-modal").hidden) closeModal(); if ((event.metaKey || event.ctrlKey) && event.key.toLowerCase() === "z") { event.preventDefault(); event.shiftKey ? redo() : undo(); } });
}
